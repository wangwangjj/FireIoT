...
class Server(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        while True:
            ...
            mydata=msg.hex()
            print('->msg:', mydata)
            # machinetype =((int(mydata[0])*16+int(mydata[1],16)))#获得机器类型
            ...
            elif((int(mydata[0])*16+int(mydata[1],16)) == 28):#用户信息传输装置操作信息
                item = "本机"
            ...
            elif((int(mydata[0])*16+int(mydata[1],16)) == 21):#时间
                item = "本机"
                state = "时间上传"
                addr = ""
